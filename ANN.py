...
for i in range(1,200):
    m.append(i)
    regr = MLPRegressor(random_state=1,max_iter=i,solver='lbfgs').fit(X_train, y_train)
    pre = []
    for X in X_test:
        pre.append(regr.predict([X]))
    mm.append(mean_absolute_error(y_test,pre))
plt.plot(m , mm)
plt.ylabel('ANN MAE')
plt.xlabel('epoch')
plt.savefig('ANN1.png')
result = pd.DataFrame({'test':y_test , 'ANN':pre})
print(result)
